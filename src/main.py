...
def main():
    ...
    clear_terminal()
    parser = argparse.ArgumentParser(description="LearnChart Trading Agent")
    parser.add_argument(
        "--assets",
        type=str,
        nargs="+",
    ...
    parser.add_argument(
        "--interval", type=str, required=False, help="Interval period, e.g., 1h"
    )
    ...
    if not args.assets or not args.interval:
        parser.error(
            "Please provide --assets and --interval, or set ASSETS and INTERVAL in .env"
        )
    ...
    active_trades = (
        []
    )  # {'asset','is_long','amount','entry_price','tp_oid','sl_oid','exit_plan'}
    ...
    print(
        f"Starting trading agent for assets: {args.assets} at interval: {args.interval}"
    )
    ...
    async def run_loop():
        ...
        while True:
            ...
            total_value = state.get("total_value") or state["balance"] + sum(
                p.get("pnl", 0) for p in state["positions"]
            )
            ...
            try:
                ...
                for ptc in positions_to_close:
                    ...
                    is_long = ptc["is_long"]
                    add_event(
                        f"RISK FORCE-CLOSE: {coin} at {ptc['loss_pct']}% loss (PnL: ${ptc['pnl']})"
                    )
                    try:
                        if is_long:
            ...
            try:
                ...
                for pos in state["positions"]:
                    try:
                        ...
                        continue
                assets_with_orders = {
                    o.get("coin") for o in (open_orders or []) if o.get("coin")
                }
                for tr in active_trades[:]:
                    asset = tr.get("asset")
                    if (
                        asset not in assets_with_positions
                        and asset not in assets_with_orders
                    ):
                        add_event(
                            f"Reconciling stale active trade for {asset} (no position, no orders)"
                        )
                        active_trades.remove(tr)
                        with open(diary_path, "a") as f:
                            f.write(
                            ...
                            try:
                                ...
                                if t_int > 1e12:
                                    timestamp = datetime.fromtimestamp(
                                        t_int / 1000, tz=timezone.utc
                                    ).isoformat()
                                else:
                                    timestamp = datetime.fromtimestamp(
                                        t_int, tz=timezone.utc
                                    ).isoformat()
                            except Exception:
                                timestamp = str(t_raw)
                        recent_fills_struct.append(
                            {
                                ...
                                "size": round_or_none(
                                    f_entry.get("sz") or f_entry.get("size"), 6
                                ),
                                "price": round_or_none(
                                    f_entry.get("px") or f_entry.get("price"), 2
                                ),
                            }
                        )
                    except Exception:
            ...
            for asset in args.assets:
                try:
                    ...
                    recent_mids = [
                        entry["mid"]
                        for entry in list(price_history.get(asset, []))[-10:]
                    ]
                    funding_annualized = (
                        round(funding * 24 * 365 * 100, 2) if funding else None
                    )
                    ...
                    market_sections.append(
                        {
                            ...
                            "intraday": {
                                "ema20": round_or_none(
                                    latest(intra.get("ema20", [])), 2
                                ),
                                "macd": round_or_none(latest(intra.get("macd", [])), 2),
                                "rsi7": round_or_none(latest(intra.get("rsi7", [])), 2),
                                "rsi14": round_or_none(
                                    latest(intra.get("rsi14", [])), 2
                                ),
                                "series": {
                                    "ema20": round_series(
                                        last_n(intra.get("ema20", []), 10), 2
                                    ),
                                    "macd": round_series(
                                        last_n(intra.get("macd", []), 10), 2
                                    ),
                                    "rsi7": round_series(
                                        last_n(intra.get("rsi7", []), 10), 2
                                    ),
                                    "rsi14": round_series(
                                        last_n(intra.get("rsi14", []), 10), 2
                                    ),
                                },
                            },
                            "long_term": {
                                ...
                                "macd_series": round_series(
                                    last_n(lt.get("macd", []), 10), 2
                                ),
                                "rsi_series": round_series(
                                    last_n(lt.get("rsi14", []), 10), 2
                                ),
                            },
            ...
            add_event(
                f"Combined prompt length: {len(context)} chars for {len(args.assets)} assets"
            )
            ...
            reasoning_text = (
                outputs.get("reasoning", "") if isinstance(outputs, dict) else ""
            )
            if reasoning_text:
            ...
            cycle_decisions = []
            for d in (
                outputs.get("trade_decisions", []) if isinstance(outputs, dict) else []
            ):
                ...
                try:
                    ...
                    if action in ("buy", "sell"):
                        ...
                        if order_type == "limit" and limit_price:
                            ...
                            add_event(
                                f"LIMIT {action.upper()} {asset} amount {amount:.4f} at limit ${limit_price}"
                            )
                        else:
                            order = (
                        ...
                        if output.get("tp_price"):
                            tp_order = await hyperliquid.place_take_profit(
                                asset, is_buy, amount, output["tp_price"]
                            )
                        ...
                        if output.get("sl_price"):
                            sl_order = await hyperliquid.place_stop_loss(
                                asset, is_buy, amount, output["sl_price"]
                            )
                        ...
                        add_event(
                            f"{action.upper()} {asset} amount {amount:.4f} at ~{current_price}"
                        )
                        if rationale:
    ...
    async def handle_diary(request):
        ...
        try:
            ...
            if raw or download:
                ...
                return web.Response(
                    text=data, content_type="text/plain", headers=headers
                )
            limit = int(request.query.get("limit", "200"))
            with open(diary_path, "r") as f:
                lines = f.readlines()
            start = max(0, len(lines) - limit)
            entries = [json.loads(line) for line in lines[start:]]
    ...
    async def handle_logs(request):
        ...
        try:
            ...
            with open(path, "r") as f:
                data = f.read()
            if download or (
                limit_param and (limit_param.lower() == "all" or limit_param == "-1")
            ):
                headers = {}
                if download:
    ...
    def calculate_total_return(state, trade_log):
        ...
        initial = 10000
        current = state["balance"] + sum(
            p.get("pnl", 0) for p in state.get("positions", [])
        )
