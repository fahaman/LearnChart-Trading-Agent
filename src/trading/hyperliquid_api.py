...
class HyperliquidAPI:
    ...
    def _build_clients(self):
        ...
        self.exchange = Exchange(
            self.wallet, self.base_url, account_address=self.account_address
        )
    ...
    def round_size(self, asset, amount):
        ...
        if ":" in asset:
            dex = asset.split(":")[0]
            dex_data = (
                self._hip3_meta_cache.get(dex)
                if hasattr(self, "_hip3_meta_cache")
                else None
            )
        ...
        try:
            ...
            if hasattr(self.info, "user_fills"):
                fills = await self._retry(
                    lambda: self.info.user_fills(self.query_address)
                )
                ...
                for bal in spot_state.get("balances", []):
                    if bal.get("coin") == "USDC":
                        ...
                        total_value = balance + sum(
                            p.get("pnl", 0.0) for p in enriched_positions
                        )
                        break
        ...
        if ":" in asset:
            ...
            raw = await self._retry(
                lambda: self.info.candles_snapshot(
                    asset, interval, start_time, end_time
                )
            )
        candles = []
        for c in raw:
