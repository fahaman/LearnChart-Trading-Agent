...
class RiskManager:
    ...
    def __init__(self):
        ...
        self.max_loss_per_position_pct = float(
            CONFIG.get("max_loss_per_position_pct") or 20
        )
        ...
        self.daily_loss_circuit_breaker_pct = float(
            CONFIG.get("daily_loss_circuit_breaker_pct") or 10
        )
        self.mandatory_sl_pct = float(CONFIG.get("mandatory_sl_pct") or 5)
        self.max_concurrent_positions = int(
            CONFIG.get("max_concurrent_positions") or 10
        )
        self.min_balance_reserve_pct = float(
            CONFIG.get("min_balance_reserve_pct") or 20
        )
    ...
    def check_position_size(
        self, alloc_usd: float, account_value: float
    ) -> tuple[bool, str]:
    ...
    def check_daily_drawdown(self, account_value: float) -> tuple[bool, str]:
        ...
        if self.circuit_breaker_active:
            return (
                False,
                "Daily loss circuit breaker is active — no new trades until tomorrow (UTC)",
            )
        if self.daily_high_value and self.daily_high_value > 0:
            drawdown_pct = (
                (self.daily_high_value - account_value) / self.daily_high_value
            ) * 100
    ...
    def validate_trade(
        self, trade: dict, account_state: dict, initial_balance: float
    ) -> tuple[bool, str, dict]:
        ...
        if sl_price is None:
            logging.info(
                "RISK: Auto-setting SL at %.2f (%.1f%% from entry)",
                enforced_sl,
                self.mandatory_sl_pct,
            )
