...
class TradingAgent:
    ...
    def __init__(self, hyperliquid=None):
        ...
        self.sanitize_model = (
            CONFIG.get("sanitize_model") or "claude-haiku-4-5-20251001"
        )
                ...
                if indicator == "all":
                    ...
                    result = {
                        ...
                        "histogram": {
                            ...
                            "series": last_n(
                                all_indicators.get("macd_histogram", []), 10
                            ),
                        },
                    }
                    ...
                    result = {
                        "upper": {
                            ...
                            "series": last_n(
                                all_indicators.get("bbands_upper", []), 10
                            ),
                        },
                        "middle": {
                            "latest": latest(all_indicators.get("bbands_middle", [])),
                            "series": last_n(
                                all_indicators.get("bbands_middle", []), 10
                            ),
                        },
                        "lower": {
                            "latest": latest(all_indicators.get("bbands_lower", [])),
                            "series": last_n(
                                all_indicators.get("bbands_lower", []), 10
                            ),
                        },
                    }
                    ...
                    series = (
                        _ema(closes, period)
                        if indicator == "ema"
                        else _sma(closes, period)
                    )
                    result = {
                        "latest": latest(series),
                        "series": last_n(series, 10),
                        "period": period,
                    }
                elif indicator == "rsi":
                    ...
                    result = {
                        "latest": (
                            latest(series) if isinstance(series, list) else series
                        ),
                        "series": (
                            last_n(series, 10) if isinstance(series, list) else series
                        ),
                    }
        ...
        for iteration in range(6):
            ...
            try:
                ...
                if not isinstance(parsed, dict):
                    logging.error(
                        "Expected dict, got: %s; attempting sanitize", type(parsed)
                    )
