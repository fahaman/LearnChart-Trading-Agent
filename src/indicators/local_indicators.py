...
def macd(candles: list[dict], fast: int = 12, slow: int = 26, signal: int = 9) -> dict:
    ...
    signal_line_raw = (
        ema(valid_macd, signal)
        if len(valid_macd) >= signal
        else [None] * len(valid_macd)
    )
...
def stoch_rsi(
    ...
    k_line = (
        sma(valid_k, k_smooth) if len(valid_k) >= k_smooth else [None] * len(valid_k)
    )
    ...
    full_k: list[float | None] = [None] * (
        pad + (len(valid_rsi) - len(valid_k)) + (len(valid_k) - len(k_line))
    )
...
def adx(candles: list[dict], period: int = 14) -> list[float | None]:
    ...
    for i in range(1, len(candles)):
        ...
        plus_dm = max(h - prev_h, 0) if (h - prev_h) > (prev_l - low_val) else 0
        minus_dm = max(prev_l - low_val, 0) if (prev_l - low_val) > (h - prev_h) else 0
        tr = max(h - low_val, abs(h - prev_c), abs(low_val - prev_c))
    ...
    for i in range(period, len(tr_list)):
        ...
        minus_dm_smooth = (
            minus_dm_smooth - (minus_dm_smooth / period) + minus_dm_list[i]
        )
